...
static uint8 rx_poll_msg[] = {0x41, 0x88, 0, 0xCA, 0xDE, 'W', 'A', 'V', 'E', 0xE0, '0', 0, 0};
...
#define DW_ID_TS_IDX 10 // ID of the module sending the frame
...
#define RX_BUF_LEN 24 + ADDED_VARIABLES_TO_MSG
...
int ss_resp_run(char* dest)
{
  ...
  if (status_reg & SYS_STATUS_RXFCG)
  {
    ...
    * As the sequence number field of the frame is not relevant, it is cleared to simplify the validation of the frame. */
    ...
    if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0 && rx_buffer[DW_ID_TS_IDX] == dest[0]) // same starting block ?
    {
      uint32 resp_tx_time;
      int ret;
      ...
      tx_resp_msg[DW_ID_TS_IDX] = dest[0];
