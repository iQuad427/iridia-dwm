...
int main(void)
{
  ...
  * As this example only handles one incoming frame with always the same delay and timeout, those values can be set here once for all. */
  ...
  // receiving
  //uint8 header_buffer[0];
  //uint8 read_buffer[sizeof(uint8)];

  // readfromspi(0, &header_buffer, sizeof(uint8), &read_buffer);

  //printf("Buffer : %d\r\n", read_buffer);

  //// acknowledging

  //writetospi(0, &header_buffer, sizeof(uint8), &read_buffer);
  
  /* Using UART.c */
  char idbuff[1];
  //memcpy(idbuff, 0, sizeof(char));
  //printf("Buffer = %c\r\n", idbuff[0]);
