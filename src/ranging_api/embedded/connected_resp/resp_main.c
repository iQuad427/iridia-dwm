    ...
    if (status_reg & SYS_STATUS_RXFCG) {
        ...
        if (frame_len <= RX_BUFFER_LEN) {
                    ...
                    ) // Check source module ID, respond if we are destination of poll or it was broadcasted
            {
                ...
                ret = dwt_starttx(
                        DWT_START_TX_DELAYED); // Delayed to match antenna delay and have more accurate ranging (because known delay)
            ...
            if (memcmp(rx_data_buffer, rx_info_msg, ALL_MSG_COMMON_LEN) == 0) {
